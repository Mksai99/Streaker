...
import { Flame, Mail, Lock, Loader2, ArrowRight, ShieldCheck, User } from 'lucide-react';
...
export default function LoginPage() {
  ...
  return (
    <div className="min-h-[100dvh] flex flex-col max-w-md mx-auto relative overflow-hidden">
      ...
      <div className="absolute top-0 right-0 w-72 h-72 bg-indigo-400/10 rounded-full blur-3xl -translate-y-1/2 translate-x-1/2" />
        ...
        <motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} className="w-full space-y-7">
          ...
          <div className="text-center space-y-3">
            <motion.div 
              ...
              className="w-20 h-20 mx-auto rounded-3xl bg-gradient-to-br from-indigo-500 via-purple-500 to-pink-500 flex items-center justify-center shadow-2xl shadow-indigo-500/30"
            >
              <Flame className="w-10 h-10 text-white" />
            </motion.div>
            <div>
              <h1 className="text-3xl font-black text-gray-900 dark:text-white tracking-tight">Customer Login</h1>
              <p className="text-sm text-gray-500 dark:text-gray-400 mt-1.5 font-medium">
                {isLogin ? 'Sign in to track your streaks' : 'Create an account to start earning'}
              </p>
            </div>
          </div>
          ...
          <motion.div className="space-y-5">
            ...
            <form onSubmit={handleSubmit} className="space-y-4">
                  ...
                  <motion.div initial={{ height: 0, opacity: 0 }} animate={{ height: 'auto', opacity: 1 }} exit={{ height: 0, opacity: 0 }} className="overflow-hidden">
                    ...
                    <div className="relative mb-4">
                      ...
                      <input type="text" value={displayName} onChange={e => setDisplayName(e.target.value)} required={!isLogin}
                        className="w-full pl-12 pr-4 py-4 rounded-2xl bg-white/70 dark:bg-gray-800/70 backdrop-blur-sm ring-1 ring-gray-200 dark:ring-gray-700 text-gray-900 dark:text-white focus:ring-2 focus:ring-indigo-500 outline-none font-medium transition-all"
              ...
              <button type="submit" disabled={loading}
                className="w-full py-4 mt-2 bg-gradient-to-r from-indigo-600 to-purple-600 hover:from-indigo-500 hover:to-purple-500 text-white rounded-2xl font-bold text-base shadow-lg shadow-indigo-500/25 active:scale-[0.98] transition-all duration-200 disabled:opacity-40 disabled:shadow-none flex items-center justify-center gap-2">
            ...
            <button type="button" onClick={() => setIsLogin(!isLogin)} disabled={loading} className="w-full text-center text-sm text-indigo-500 dark:text-indigo-400 font-semibold hover:text-indigo-600 transition-colors">
              {isLogin ? "Don't have an account? Sign Up" : "Already have an account? Sign In"}
            </button>

            <p className="text-center text-[11px] text-gray-400 dark:text-gray-500 flex items-center justify-center gap-1">
              <ShieldCheck className="w-3.5 h-3.5" /> Secure Customer Portal
            </p>
          </motion.div>
        </motion.div>
      </div>
